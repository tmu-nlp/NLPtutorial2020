...
class DataLoader(object):
    def __init__(self, text):
        self.v_dict = defaultdict(lambda x: 0)
        self.total_len = 1
        self.v_dict['<UNK>'] = 0
        for sent in text:
            for word in sent:
                if word not in self.v_dict:
                    self.v_dict[word] = self.total_len
                    self.total_len += 1
...
if __name__ == '__main__':
    ...
    pos_loader = DataLoader(pos)

    model = RNN()
    model.train_with_sgd(text_loader, pos_loader)
