...
class hmm:
            ...
            for i in range(I):
                for prev in possible_tags:
                    ...
                    for next in possible_tags:
                        ...
                        if (i, prev) in best_score and transition[(prev,next)] != False:
                            #print("Yes")
                            #print(transition[(prev, next)])
                            #print(words[i], next)
                            #print(emission[(next, words[i])])
                            score = best_score[(i, prev)] - log2(transition[(prev, next)]) - log2((0.95*emission[(next, words[i])] + 0.05/1000000))
                            #print(score)
                    ...
                    if (i, prev) in best_score and transition[(prev, "</s>")] != False:
                        score = best_score[(i, prev)] + log2(transition[(prev, "</s>")]) + log2((0.95*emission[("</s>", words[i])] + 0.05/1000000))
                        if (i+1, "</s>") not in best_score or best_score[i+1, "</s>"] > score:
    ...
    def train_hmm(self, train_file, output):
        ...
        for key in transition:
            ...
            output_file.write("T " + key +" "+str(transition[key]/context[previous]) +"\n")  
        # 生成確率を出力
        for key in emit:
            previous, word = key.split()
            output_file.write("E "+ key +" "+ str(emit[key]/context[previous]) + "\n")
        output_file.close()
